package com.example.androiddemo.ui.map.widget
...
object MapHelper {
     ...
     * 腾讯地图App包名
     */
    const val TENCENT_MAP_PACKAGE_NAME = "com.tencent.map"
    ...
    fun gotoMap(context: Context, toLatitude: Double?, toLongitude: Double?, mapType: Int = MapType.UNSPECIFIED_MAP_TYPE, isMarket: Boolean = false, isContainGoogle: Boolean = true): Boolean {
        return when (mapType) {
            MapType.UNSPECIFIED_MAP_TYPE -> gotoMap(context, toLatitude, toLongitude, isMarket, isContainGoogle)
            MapType.AMAP_TYPE -> gotoAMap(context, toLatitude, toLongitude, isRoute = true, isMarket = isMarket)
            MapType.BAIDU_MAP_TYPE -> gotoBaiduMap(context, toLatitude, toLongitude, isRoute = true, isMarket = isMarket)
            MapType.TENCENT_MAP_TYPE -> gotoTencentMap(context, toLatitude, toLongitude, isMarket = isMarket)
     ...
     * @return 返回true表示满足条件并执行了跳转操作（跳转到相关地图App或跳转到应用市场对应App详情）
     */
    private fun gotoMap(context: Context, toLatitude: Double?, toLongitude: Double?, isMarket: Boolean = false, isContainGoogle: Boolean = true): Boolean {
        when {
            isInstalled(context, AMAP_PACKAGE_NAME) -> {
                return gotoAMap(context, toLatitude, toLongitude, isRoute = true, isMarket = isMarket)
            }
            isInstalled(context, BAIDU_MAP_PACKAGE_NAME) -> {
                return gotoBaiduMap(context, toLatitude, toLongitude, isRoute = true, isMarket = isMarket)
            }
    ...
    fun gotoAMap(
        ...
        return if (isRoute) {
            gotoAMapRoute(context, toLatitude = toLatitude, toLongitude = toLongitude, isWGS84 = isWGS84, source = source, isMarket = isMarket, marketPackage = marketPackage)
        } else {
            gotoAMapNavigation(context, toLatitude, toLongitude, isWGS84, source, isMarket, marketPackage)
        }
    }
    ...
    fun gotoAMapNavigation(
        ...
        val uri = "androidamap://navi?sourceApplication=${source}&lat=${toLatitude}&lon=${toLongitude}&dev=${dev}&style=2"
    ...
    fun gotoAMapRoute(
        ...
        return if (fromLatitude == null || fromLongitude == null) {//起点经纬度为空时，表示从当前位置到终点位置
            if (toLatitude == null || toLongitude == null) {//终点经纬度为空时，表示从当前位置到
                val uri = "amapuri://route/plan/?sourceApplication=${source}&dev=${dev}&t=${mode}"
                gotoUri(context, AMAP_PACKAGE_NAME, Uri.parse(uri), isMarket, marketPackage)
            } else {
                val uri = "amapuri://route/plan/?sourceApplication=${source}&dlat=${toLatitude}&dlon=${toLongitude}&dev=${dev}&t=${mode}"
                gotoUri(context, AMAP_PACKAGE_NAME, Uri.parse(uri), isMarket, marketPackage)
            }
        } else {
            if (toLatitude == null || toLongitude == null) {//终点经纬度为空时，表示从当前位置到
                val uri = "amapuri://route/plan/?sourceApplication=${source}&slat=${fromLatitude}&slon=${fromLongitude}&dev=${dev}&t=${mode}"
                gotoUri(context, AMAP_PACKAGE_NAME, Uri.parse(uri), isMarket, marketPackage)
            } else {
                val uri = "amapuri://route/plan/?sourceApplication=${source}&slat=${fromLatitude}&slon=${fromLongitude}&dLat=${toLatitude}&dlon=${toLongitude}&dev=${dev}&t=${mode}"
                gotoUri(context, AMAP_PACKAGE_NAME, Uri.parse(uri), isMarket, marketPackage)
            }
        }
    }
    ...
    fun gotoBaiduMapRoute(
        ...
        return if (fromLatitude == null || fromLongitude == null) {//起点经纬度为空时，表示从当前位置到终点位置
            ...
            if (toLatitude == null || toLongitude == null) {//终点经纬度为空时，表示从当前位置到
                val uri = "baidumap://map/direction?origin=${fromLatitude},${fromLongitude}&coord_type=${coordinateType}&mode=${tripMode}&src=${source}"
                gotoUri(context, BAIDU_MAP_PACKAGE_NAME, Uri.parse(uri), isMarket, marketPackage)
            } else {
    ...
    fun gotoTencentMap(context: Context, toLatitude: Double?, toLongitude: Double?, referer: String = DEFAULT_REFERER, isMarket: Boolean = false, marketPackage: String? = null): Boolean {
        return gotoTencentMapRoute(context, null, null, toLatitude, toLongitude, TripMode.DRIVING_MODE, referer, isMarket, marketPackage)
    }
    ...
    fun gotoTencentMapRoute(
        ...
        return if (fromLatitude == null || fromLongitude == null) {//起点经纬度为空时，表示从当前位置到终点位置
            if (toLatitude == null || toLongitude == null) {//终点经纬度为空时，表示从当前位置到
                val uri = "qqmap://map/routeplan?type=${tripMode}&referer=${referer}"
                gotoUri(context, TENCENT_MAP_PACKAGE_NAME, Uri.parse(uri), isMarket, marketPackage)
            } else {
                val uri = "qqmap://map/routeplan?type=${tripMode}&tocoord=${toLatitude},${toLongitude}&referer=${referer}"
                gotoUri(context, TENCENT_MAP_PACKAGE_NAME, Uri.parse(uri), isMarket, marketPackage)
            }
        } else {
            if (toLatitude == null || toLongitude == null) {//终点经纬度为空时，表示从当前位置到
                val uri = "qqmap://map/routeplan?type=${tripMode}&fromcoord=${fromLatitude},${fromLongitude}&referer=${referer}"
                gotoUri(context, TENCENT_MAP_PACKAGE_NAME, Uri.parse(uri), isMarket, marketPackage)
            } else {
                val uri = "qqmap://map/routeplan?type=${tripMode}&fromcoord=${fromLatitude},${fromLongitude}&tocoord=${toLatitude},${toLongitude}&referer=${referer}"
                gotoUri(context, TENCENT_MAP_PACKAGE_NAME, Uri.parse(uri), isMarket, marketPackage)
            }
        }
    }
    ...
    fun gotoGoogleMapNavigation(context: Context, toLatitude: Double?, toLongitude: Double?, mode: Int = TripMode.DRIVING_MODE, isMarket: Boolean = false, marketPackage: String? = null): Boolean {
        ...
        return if (toLatitude == null || toLongitude == null) {//终点经纬度为空时，表示从当前位置到
            val uri = "google.navigation:mode=${tripMode}"
            gotoUri(context, GOOGLE_MAP_PACKAGE_NAME, Uri.parse(uri), isMarket, marketPackage)
        } else {
